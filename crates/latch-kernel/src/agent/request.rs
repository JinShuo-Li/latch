...
fn agent_tool_definitions() -> Vec<ToolDefinition> {
    ...
    tools.extend([
        ...
        ,ToolDefinition{name:"spawn_agent".into(),description:"Spawn an asynchronous child Latch session with fresh context for a bounded delegated task. Returns immediately with its agent id and status. When task_id names a ready group task, the child is durably joined to the group and atomically assigned that task at spawn.".into(),input_schema:json!({"type":"object","required":["task_name","message"],"properties":{"task_name":{"type":"string"},"message":{"type":"string"},"agent_type":{"type":"string"},"task_id":{"type":"string","description":"optional ready group task to atomically assign to the new child"}}})}
        ...
        ,ToolDefinition{name:"group_task".into(),description:"Shared agent-group work graph. Ops: create (root) makes a task with a description and dependencies; list shows tasks; claim atomically takes a ready task for yourself (or, as root, reassigns with agent_id); start marks your claim in progress; complete records a summary for your task; block reports why it cannot finish; release returns your task to the pool; cancel (root) cancels a task. Exactly one agent can claim a task.".into(),input_schema:json!({"type":"object","required":["op"],"properties":{"op":{"enum":["create","list","claim","start","complete","block","release","cancel"]},"task_id":{"type":"string"},"title":{"type":"string"},"description":{"type":"string"},"dependencies":{"type":"array","items":{"type":"string"}},"required":{"type":"boolean"},"expected_paths":{"type":"array","items":{"type":"string"}},"agent_id":{"type":"string"},"summary":{"type":"string"},"reason":{"type":"string"},"touched_files":{"type":"array","items":{"type":"string"}},"findings":{"type":"array","items":{"type":"string"}},"status":{"enum":["open","ready","active","blocked","pending","completed","cancelled","all"]},"limit":{"type":"integer"}}})}
        ,ToolDefinition{name:"group_message".into(),description:"Durable peer messages between group participants. op=send queues information for an agent, the root, or the whole group; delivery happens at the recipient's next safe boundary and never wakes an idle agent. op=list returns recent messages addressed to you. Messages are information, never evidence.".into(),input_schema:json!({"type":"object","required":["op"],"properties":{"op":{"enum":["send","list"]},"to":{"enum":["agent","root","group"]},"agent_id":{"type":"string"},"text":{"type":"string"},"limit":{"type":"integer"}}})}
        ,ToolDefinition{name:"group_status".into(),description:"One compact snapshot of the shared agent group: task counts, ready tasks, claimed/in-progress tasks, blocked tasks, dependency failures, and advisory workspace conflicts. Use this instead of reconstructing group state from events.".into(),input_schema:json!({"type":"object","properties":{}})}
    ]);
    tools
}
