...
fn agent_tool_definitions() -> Vec<ToolDefinition> {
    ...
    tools.extend([
        ...
        ToolDefinition{name:"complete".into(),description:"State that implementation work is done. The kernel derives completion (Verified / ImplementedNotVerified / Blocked / InProgress) from this claim and current evidence.".into(),input_schema:json!({"type":"object","required":["implementation_done"],"properties":{"implementation_done":{"type":"boolean"}}})}
        ,ToolDefinition{name:"spawn_agent".into(),description:"Spawn an asynchronous child Latch session with fresh context for a bounded delegated task; returns its id and status immediately. When task_id names a ready group task, the child joins the group and is atomically assigned it at spawn.".into(),input_schema:json!({"type":"object","required":["task_name","message"],"properties":{"task_name":{"type":"string"},"message":{"type":"string"},"agent_type":{"type":"string"},"task_id":{"type":"string","description":"optional ready group task to atomically assign to the new child"}}})}
        ...
        ,ToolDefinition{name:"wait_agents".into(),description:"Wait for final states or updates from selected children (all when agent_ids is omitted). Returns current statuses; completed reports arrive separately as kernel notifications at the next model boundary. A timeout returns current statuses.".into(),input_schema:json!({"type":"object","properties":{"agent_ids":{"type":"array","items":{"type":"string"}},"timeout_ms":{"type":"integer","minimum":0,"maximum":300000}}})}
        ...
        ,ToolDefinition{name:"group_task".into(),description:"Shared agent-group work graph. Ops: create (root) adds a task with description/dependencies; list shows tasks; claim atomically takes a ready task (root may reassign via agent_id); start marks your claim in progress; complete records a summary; block reports why it cannot finish; release returns it to the pool; cancel (root) cancels. Exactly one agent can claim a task.".into(),input_schema:json!({"type":"object","required":["op"],"properties":{"op":{"enum":["create","list","claim","start","complete","block","release","cancel"]},"task_id":{"type":"string"},"title":{"type":"string"},"description":{"type":"string"},"dependencies":{"type":"array","items":{"type":"string"}},"required":{"type":"boolean"},"expected_paths":{"type":"array","items":{"type":"string"}},"agent_id":{"type":"string"},"summary":{"type":"string"},"reason":{"type":"string"},"touched_files":{"type":"array","items":{"type":"string"}},"findings":{"type":"array","items":{"type":"string"}},"status":{"enum":["open","ready","active","blocked","pending","completed","cancelled","all"]},"limit":{"type":"integer"}}})}
        ,ToolDefinition{name:"group_message".into(),description:"Durable peer messages between group participants. op=send queues information for an agent, the root, or the group; delivery is at the recipient's next safe boundary and never wakes an idle agent. op=list returns recent messages to you. Messages are information, never evidence.".into(),input_schema:json!({"type":"object","required":["op"],"properties":{"op":{"enum":["send","list"]},"to":{"enum":["agent","root","group"]},"agent_id":{"type":"string"},"text":{"type":"string"},"limit":{"type":"integer"}}})}
        ,ToolDefinition{name:"group_status".into(),description:"One compact snapshot of the shared group: task counts, ready/claimed/blocked tasks, dependency failures, advisory workspace conflicts. Prefer this over reconstructing group state from events.".into(),input_schema:json!({"type":"object","properties":{}})}
    ]);
    tools
}
...
pub(crate) fn context_messages(ctx: &crate::continuity::MaterializedContext) -> Vec<ModelMessage> {
    let raw = ctx
        ...
        .filter_map(|e| match &e.payload {
            ...
            EventPayload::GroupMessageDelivered { message } => Some(ModelMessage::text(
                "user",
                format!(
                    "Kernel group message from {} to {} ({}; information only, never evidence):\n{}",
                    message.from_agent,
                    match &message.to {
                        latch_protocol::GroupMessageTarget::Agent(agent) => agent.to_string(),
                        latch_protocol::GroupMessageTarget::Root => "root".to_owned(),
                        latch_protocol::GroupMessageTarget::Group => "group".to_owned(),
                    },
                    message.created_at.format("%Y-%m-%dT%H:%M:%SZ"),
                    message.text,
                ),
            )),
